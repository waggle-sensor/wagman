...
void WagID_print()
{
    byte k;
    RTC.idRead(WagID);
    Serial.print("0. Unique Board ID # ");
    for (k = 0; k<0x08; k++)
    {
        if (WagID[k] < 16)
            Serial.print("0");
...
void thermistor_check()
{
    unsigned int val;
    
    val =analogRead(PIN_Therm_NC);
    mcp3428_1.selectChannel(MCP342X::CHANNEL_0, MCP342X::GAIN_1);
    val+=mcp3428_1.readADC()>>5;
    ...
    if (val < 10)
    {
        Serial.println("II. Thermistors: PASS");
    }
    else 
    {
        Serial.println("II. Thermistors: FAIL");
    }
...
void boot_pow_check()
{
    ...
    Serial.println("We will power ON all relays one by one. You should hear 5 seperate clicks.");
