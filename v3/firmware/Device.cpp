...
// const unsigned long HEARTBEAT_TIMEOUT = 300000L; // 5m
...
byte Device::start() {
  ...
  managed = Record::getBootFailures(port) < 1024;
...
byte Device::stop() {
  if (state != STATE_STARTED) return ERROR_INVALID_ACTION;
...
byte Device::kill() {
  if (state == STATE_DISABLED) return ERROR_INVALID_ACTION;
...
void Device::updateHeartbeat() {
  // ...this is probably all we'd like exposed from the outside...not this
  // combination...
