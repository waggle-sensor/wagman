/*
 ...
 This library allows an Arduino to read from the HTU21D low-cost high-precision
 humidity sensor.

 If you have feature suggestions or need support please use the github support
 page: https://github.com/sparkfun/HTU21D

 Hardware Setup: The HTU21D lives on the I2C bus. Attach the SDA pin to A4, SCL
 to A5. If you are using the SparkFun breakout board you *do not* need 4.7k
 ...
 HTU21D.ReadTemperature() will return a float containing the temperature in
 Celsius. Ex: 24.1 HTU21D.SetResolution(byte: 0b.76543210) sets the resolution
 of the readings. HTU21D.check_crc(message, check_value) verifies the 8-bit CRC
 generated by the sensor HTU21D.read_user_register() returns the user register.
 Used to set resolution.
 */
...
float HTU21D::readHumidity(void) {
  ...
  Wire.write(
      TRIGGER_HUMD_MEASURE_NOHOLD);  // Measure humidity with no bus holding
...
void HTU21D::setResolution(byte resolution) {
  byte userRegister = read_user_register();  // Go get the current register
                                             // state
...
// Give this function the 2 byte message (measurement) and the check_value byte
// from the HTU21D If it returns 0, then the transmission was good If it returns
// something other than 0, then the communication was corrupted From:
// http://www.nongnu.org/avr-libc/user-manual/group__util__crc.html POLYNOMIAL =
// 0x0131 = x^8 + x^5 + x^4 + 1 :
// http://en.wikipedia.org/wiki/Computation_of_cyclic_redundancy_checks
#define SHIFTED_DIVISOR \
  0x988000  // This is the 0x0131 polynomial shifted to farthest left of three
            // bytes

byte HTU21D::check_crc(uint16_t message_from_sensor,
  ...
  for (int i = 0; i < 16;
       i++)  // Operate on only 16 positions of max 24. The remaining 8 are our
             // remainder and should be zero when we're done.
  {
    if (remainder & (uint32_t)1
                        << (23 -
                            i))  // Check if there is a one in the left position
      remainder ^= divsor;

    divsor >>= 1;  // Rotate the divsor max 16 times so that we have 8 bits left
                   // of a remainder
  }
